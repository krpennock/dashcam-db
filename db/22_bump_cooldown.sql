...
DECLARE
  ...
  bump_cooldown_s integer := GREATEST(COALESCE((p_params->>'bump_cooldown_s')::int, 3), 0);
BEGIN
  ...
  IF bump_p99 IS NOT NULL AND bump_p999 IS NOT NULL THEN
    ...
    b3 AS (
      SELECT
        b2.*,
        max(CASE WHEN is_bump THEN ts_s END)
          OVER (PARTITION BY drive_session_id ORDER BY ts_s
                ROWS BETWEEN UNBOUNDED PRECEDING AND 1 PRECEDING) AS prev_bump_ts
      FROM b2
    )
    INSERT INTO dashcam.derived_event(drive_session_id, ts_utc, event_type, severity, details)
    SELECT
      ...
      'bump',
      (max_mag - bump_p99),
      jsonb_build_object('max_mag', max_mag, 'thr', bump_thr, 'p99', bump_p99, 'p999', bump_p999, 'cooldown_s', bump_cooldown_s)
    FROM b3
    WHERE is_bump
      AND (
        prev_bump_ts IS NULL
        OR (ts_s - prev_bump_ts) >= (bump_cooldown_s * interval '1 second')
      );
  END IF;
